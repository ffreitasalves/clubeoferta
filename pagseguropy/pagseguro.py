...
class Pagseguro:
  ...
  def mostra(self, **kws):
    ...
    ret = []
    if data['abre']:
      # a linha seguinte foi usada para testes com o PagSeguro Server de ASP.net
      #ret.append(u'<form action="http://localhost:9090/security/webpagamentos/webpagto.aspx" target="pagseguro" method="post"%s>' % ('extra' in data and data['extra']))
      ret.append(u'<form action="https://pagseguro.uol.com.br/security/webpagamentos/webpagto.aspx" target="pagseguro" method="post"%s>' % ('extra' in data and data['extra']))
